...
def start_end_label(Dataframe, number_row): 
    ...
    return [(entry["value"]["start"],                                 #List comprehension. We store the start, end, text, number of words in the text and the label for each of the texts in the text
             entry["value"]["end"],
...
def get_label_in_text(Dataframe, number_row, Show=False):
    ...
    Text_and_Labels = pd.DataFrame([(number_row, text[pred[0]:pred[1]], pred[0], pred[1], pred[2]) for pred in Predictions], columns=['File', 'Text','Start', 'End', 'Label'])
